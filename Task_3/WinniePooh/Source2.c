/*
 * Сложить две матрицы и сохранить результат в третьей результирующей матрице
 * Простой алгоритм: сколько строк в матрице, столько потоков.
 ...
 * То есть, каждый поток работает ровно со своими данными
...
void* threadFunc(void* thread_data) {
	...
	for (int i = 0; i < data->rowSize; i++) {
		data->resArr[data->rowN][i] =
			data->array1[data->rowN][i] +
			data->array2[data->rowN][i];
	}
	return NULL;
}

void PrintMatrix(const char* title, int** matrix) {
	printf("------------------\n%s\n------------------\n", title);
